...
pontos = lambda reg: (reg[1] * 3) + reg[2]
...
def main():
    ...
    tab = sorted(tabela, reverse=True, key=pontos)
    printTabela(tab)

    # c)
    # Acrescente os argumentos adequados à função sorted para
    # obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
    tab = sorted(tabela, reverse=True, key=lambda x: x[4]- x[5])
    printTabela(tab)
    ...
    tab = sorted(tabela, reverse=True, key=lambda x:(pontos, x[4]- x[5]))
    printTabela(tab)
